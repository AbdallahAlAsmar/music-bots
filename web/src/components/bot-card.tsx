...
import { BulkSelectionHint } from "@/components/bulk-selection-bar";
import { effectiveBotStatus, StatusBadge } from "@/components/status-badge";
import { Equalizer } from "@/components/motion-primitives";
import { AlertIcon, ArrowRightIcon, MicIcon } from "@/components/icons";
...
export function BotCard({ bot }: BotCardProps) {
  const { selectionMode, isSelected, toggleSelected } = useBots();
    ...
    <>
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-center gap-4">
          <div className="relative shrink-0">
            <BotAvatar bot={bot} size="md" className="ring-1 ring-white/10" />
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        ...
        {!selectionMode ? (
          ...
          <span className={`text-xs font-medium ${selected ? "text-emerald-300" : "text-slate-500"}`}>
  ...
  if (selectionMode) {
    return (
      <motion.button
        ...
        className={`card group block h-full w-full cursor-pointer p-5 text-left transition-colors duration-200 ${
          selected ? "border-emerald-500/50 bg-emerald-500/5" : "hover:border-emerald-500/40"
        }`}
      >
        {inner}
      </motion.button>
